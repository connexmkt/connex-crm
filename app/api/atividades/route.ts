/**
 * GET  /api/atividades  — lista as atividades mais recentes
...
export async function GET(request: Request) {
  ...
  const associacaoTipo = searchParams.get('associacaoTipo') as AtividadeAssociacaoTipo | null
...
export async function POST(request: Request) {
  ...
  const TIPOS_VALIDOS: AtividadeTipo[] = ['reuniao', 'ligacao', 'email', 'mensagem', 'proposta', 'contrato']
  ...
  if (!associacaoTipo || !ASSOC_VALIDOS.includes(associacaoTipo as AtividadeAssociacaoTipo)) {
  ...
  if (!associacaoNome) return badRequest('Campo "associacaoNome" é obrigatório')
