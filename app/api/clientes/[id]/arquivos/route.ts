...
export async function GET(_request: NextRequest, { params }: RouteContext) {
  ...
  try {
    const arquivos = await ArquivosService.listByCliente(
      supabase,
      idParsed.data,
    );
    return ok(arquivos);
...
export async function POST(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    return badRequest({
      message: "Corpo da requisição deve ser multipart/form-data",
    });
  }
  ...
  if (!(file instanceof File))
    return badRequest({ message: 'Campo "file" é obrigatório' });
  if (typeof name !== "string" || name.trim().length === 0) {
    return badRequest({ message: 'Campo "name" é obrigatório' });
  }
  if (
    !FILE_TYPE_VALUES.includes(fileType as (typeof FILE_TYPE_VALUES)[number])
  ) {
    return badRequest({
