...
import { weeklyReportIngestSchema } from "@/lib/schemas/instagram-reports/weekly-report-ingest.schema";
 ...
 * semanais de Instagram já processados — ver contracts/instagram-reports-ingestion-api.yaml.
 */
export async function POST(request: NextRequest) {
  ...
  const parsed = weeklyReportIngestSchema.safeParse(body);
  if (!parsed.success) return badRequest(parsed.error.flatten());

  try {
    const admin = createCrmAdminClient();
    const result = await InstagramReportsIngestionService.ingestWeeklyReport(
      admin,
      parsed.data,
    );
