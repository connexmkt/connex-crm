 ...
 *   Retorna as notificações do usuário autenticado (as 30 mais recentes).
 *   Response 200: { data: Notification[] }
 *
 * PATCH /api/notifications
 *   Marca notificações como lidas.
 *   Body: { id?: string }   — omitir "id" marca TODAS como lidas.
...
export async function GET() {
  ...
  try {
    const notifications = await NotificationsService.listForUser(supabase, user.id, 30)
