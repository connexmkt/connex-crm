...
**Resultado (confirmado em `/speckit.implement`, 2026-07-22)**: o servidor MCP `supabase-insights` não estava autenticado/disponível nesta sessão de implementação, então a verificação não pôde ser feita via `list_tables`/`execute_sql` como planejado. Em vez disso, a dependência foi confirmada **diretamente no código-fonte** do `connex-insights`: `connex-insights/prisma/schema.prisma` (model `Profile`, linhas 72-85) não possui nenhum campo `login` — os únicos campos são `id`, `tenantId`, `displayName`, `role`, `status`, `createdAt`, `updatedAt`. Isso confirma que a coluna `login` (UNIQUE, NOT NULL) **ainda não existe** em `public.profiles`.

**Consequência**: `ConnexInsightsRemoteRepository.insertProfile` (`lib/repositories/connex-insights-remote.repository.ts`) já envia `login` no payload do `INSERT`, conforme o contrato desejado — mas essa chamada **falhará em produção** (coluna inexistente) até que o `connex-insights` aplique uma migration Prisma adicionando `login text UNIQUE NOT NULL` ao model `Profile` e ajuste seu fluxo de autenticação para usar `login` em vez de/além de `email`. Esta é a mesma dependência bloqueante já registrada na seção "Impacto Cross-Repo" do `spec.md` — **não implementável dentro do repositório `connex-crm`**. Recomenda-se reexecutar esta verificação via MCP `supabase-insights` assim que o servidor estiver autenticado, para confirmar o estado em produção antes do T059 (gate de release).
