...
- [~] T007 Gerar e aplicar migration Prisma (`pnpm prisma migrate dev --name add_insights_user_provisioning_requests`) — SQL da migration escrito manualmente em `prisma/migrations/20260722180000_add_insights_user_provisioning_requests/migration.sql` (formato padrão Prisma) porque não há `DATABASE_URL` real acessível neste ambiente de implementação; **`pnpm prisma migrate deploy` precisa ser executado manualmente** contra o Postgres real antes do primeiro uso
- [X] T008 [P] Adicionar SQL de RLS (`ALTER TABLE ... ENABLE ROW LEVEL SECURITY` + policies SELECT/INSERT descritas em `data-model.md`; sem policy de UPDATE/DELETE para `authenticated`) — incluído na mesma migration de T007
- [~] T009 [P] Validar o schema resultante via MCP `supabase` — **não executado**: o servidor MCP `supabase` (projeto do CRM) não estava autenticado/disponível nesta sessão; revisão manual da migration SQL feita em seu lugar. Rodar `list_tables`/`execute_sql` assim que o MCP estiver disponível
- [~] T010 [P] Verificar dependência bloqueante externa (coluna `login` em `public.profiles`) — **MCP `supabase-insights` indisponível nesta sessão**; verificado por leitura direta de `connex-insights/prisma/schema.prisma`, que confirma a ausência da coluna `login`. Resultado registrado em `research.md` § D7
...
- [ ] T057 Revisão de segurança via MCP `supabase`/`supabase-insights` — **não executado** (MCPs indisponíveis nesta sessão); revisão manual confirma que `SUPABASE_INSIGHTS_SERVICE_ROLE_KEY` só é lida em `lib/integrations/connex-insights/admin-client.ts` (módulo server-only, nunca importado por Client Components)
...
- **Execução de `/speckit.implement` (2026-07-22)**: todo o código de produção das 3 user stories foi implementado (37 arquivos novos/alterados), `pnpm build` e `pnpm exec tsc --noEmit` passam sem erros novos, e `pnpm test` roda 39 testes (8 arquivos) com sucesso. As seguintes tarefas **não puderam ser executadas** por dependerem de infraestrutura viva indisponível neste ambiente de implementação (sem `DATABASE_URL`/`SUPABASE_INSIGHTS_*` reais e sem os servidores MCP `supabase`/`supabase-insights` autenticados): T007 (aplicar migration), T009, T010 (verificado por leitura direta do schema em vez de MCP), T020, T024, T025-T027, T037 (parcial), T040-T042 (substituídos por unit tests equivalentes do service), T044, T054, T057. Ver detalhes em cada tarefa acima e no relatório final da conversa.
