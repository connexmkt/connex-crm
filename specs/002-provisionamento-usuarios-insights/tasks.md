...
- [X] T059 Confirmar status da dependência externa (T010 / `research.md` § D7 — coluna `login` no Connex Insights) antes de habilitar a feature em produção — **resolvido em 2026-07-22**: migration `add_profile_login` aplicada no `connex-insights` (coluna `profiles.login` + fluxo de login atualizado para usar `login` em vez de `email`); ver `research.md` § D7 "Resolução" para detalhes. A feature pode ser habilitada em produção
...
- **Resolução da dependência cross-repo (2026-07-22, mesmo dia)**: a coluna `login` foi adicionada a `connex-insights/profiles` e o fluxo de login daquele repositório foi atualizado para autenticar por `login` (resolvendo para e-mail internamente via Admin API). T059 está concluída; a feature não tem mais bloqueio de release conhecido. Detalhes em `research.md` § D7.
- **Remoção da guarda por papel `Admin` (2026-07-22, mesmo dia, a pedido do usuário)**: as tarefas abaixo foram implementadas originalmente com uma guarda `role === 'Admin'` (T015, T021, T022, T029-T032, T037, T049) — os itens `[X]` acima refletem esse estado histórico e não foram reescritos, para preservar o registro do que foi de fato implementado em cada sessão. O comportamento **atual** do código é diferente: `lib/auth/require-admin.ts` foi substituído por `lib/auth/require-auth.ts` (`checkAuth`/`requireAuthOrRedirect`), que exige apenas sessão autenticada, sem checagem de papel. Consequências: (1) o item "Aplicações" da sidebar (T021) é exibido para qualquer usuário autenticado; (2) as rotas de `dashboard`, `tenants` e `usuarios` (T029-T031, T049) não retornam mais `403 Forbidden`, apenas `401 Unauthorized`; (3) `tests/unit/require-admin.test.ts` (T019) foi substituído por `tests/unit/require-auth.test.ts`; `tests/integration/connex-insights-usuarios-authz.test.ts` (T043) e `tests/integration/connex-insights-dashboard-failure.test.ts` (T028) foram atualizados para remover os casos de "403 sem papel Admin"; (4) as policies de RLS de `audit_log` e `insights_user_provisioning_requests` (que já eram apenas defesa em profundidade — ver `research.md` § D8) foram atualizadas nas migrations `supabase/migrations/20260722210000_audit_log_allow_any_authenticated_insert.sql` e `prisma/migrations/20260722211500_insights_provisioning_allow_any_authenticated/migration.sql` para exigir apenas autenticação. `spec.md`, `plan.md`, `data-model.md`, `quickstart.md`, `research.md` e o contrato OpenAPI foram atualizados com a mesma data para refletir o novo comportamento (FR-004, SEC-002, SC-005 e as respostas `403` dos endpoints).
